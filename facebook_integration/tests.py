...
class ViewsTest(TestCase):
    ...
    def test_create_template_post(self):
        ...
        response = self.client.post(
            reverse('facebook_integration:create_template'),
            data
        )
        
        self.assertEqual(response.status_code, 302)  # Redirect após sucesso
        self.assertTrue(
            PostTemplate.objects.filter(name='Novo Template').exists()
        )
...
class TasksTest(TestCase):
    ...
    @patch('facebook_integration.services.openai_service.OpenAIService.generate_post_content')
    @patch('facebook_integration.services.openai_service.OpenAIService.generate_image_prompt')
...
class IntegrationTest(TestCase):
    ...
    def test_complete_post_workflow(self):
        ...
        response = self.client.post(
            reverse('facebook_integration:create_scheduled_post'),
            data
        )
        ...
        with patch('facebook_integration.services.openai_service.OpenAIService.generate_post_content') as mock_content:
            with patch('facebook_integration.services.openai_service.OpenAIService.generate_image_prompt') as mock_image:
