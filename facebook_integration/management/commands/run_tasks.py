...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--post-id", type=int, help="ID do post para ações específicas"
        )
        parser.add_argument(
            "--template-id", type=int, help="ID do template para criar posts"
        )
    ...
    def generate_content(self, post_id):
        ...
        if not post_id:
            self.stdout.write(self.style.ERROR("É necessário fornecer --post-id"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Task iniciada: {task.id}"))
        except ScheduledPost.DoesNotExist:
            self.stdout.write(self.style.ERROR(f"Post {post_id} não encontrado"))
    ...
    def list_active_tasks(self):
        ...
        for task in active_tasks:
            ...
            self.stdout.write(f"🔄 {task.task_name} | {task.status}{duration}")
